# Usage: python patches_store.py -mm="ram" --debug=1 -pl 32 -po 0 \
# -ts 0 -tnl 10000 -bs=5000 --batch_size=128 --filters=256 \
# -m="smcnn_semantic" --phase="repeat" -sc=False --class_n=12 \
# --log_dir="../data/summaries/" --path="../cv_data/" --im_h=8492 \
# --im_w=7995 --band_n=2 --t_len=7 --id_first=8 -tof=False -nap=10000 \
# -psv=True
# Only relevant: pl, class_n, path, im_h, im_w, band_n, t_len
...
def main(_):
    ...
    with tf.Session() as sess:
        ...
        if args.model=='convlstm':
            ...
            model = SMCNN_semantic(sess, batch_size=args.batch_size, epoch=args.epoch, train_size=args.train_size,
                            timesteps=args.timesteps, patch_len=args.patch_len,
                            kernel=args.kernel, channels=args.channels, filters=args.filters, n_classes=args.n_classes,
                            checkpoint_dir=args.checkpoint_dir,log_dir=args.log_dir,data=data.ram_data,conf=data.conf, debug=args.debug)
